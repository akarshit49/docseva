...
CO_GSTIN = ""
CO_PHONE = ""
CO_LOCATION = ""   # "City, State" — shown as subtitle in header
CO_WEBSITE = ""    # shown in footer when set, replaces hardcoded industry tagline
...
def _draw_header(pdf: "FPDF") -> None:
    ...
    # Third line in header: city/state location, or website, or nothing — never a hardcoded tagline.
    subtitle3 = CO_LOCATION or CO_WEBSITE
    if subtitle3:
        pdf.set_xy(name_x, 37)
        pdf.set_font("Helvetica", "", 8)
        pdf.set_text_color(*GOLD_RGB)
        pdf.cell(name_w, 6, _s(subtitle3), align="C")
...
def _draw_footer(pdf: "FPDF") -> None:
    ...
    # Footer first line: company name, with website appended if configured.
    footer_line1 = CO_NAME
    if CO_WEBSITE:
        footer_line1 = f"{CO_NAME}  \u2014  {CO_WEBSITE}"
    pdf.cell(210, 7, _s(footer_line1), align="C")
...
def generate_catalog(
    ...
    """
    global CO_NAME, CO_ADDRESS, CO_GSTIN, CO_PHONE, CO_LOCATION, CO_WEBSITE, USER_LOGO_PATH
    if company_profile:
        CO_NAME = company_profile.get("display_name") or CO_NAME
        city = company_profile.get("city") or ""
        state = company_profile.get("state") or ""
        CO_LOCATION = ", ".join(p for p in [city, state] if p)
        addr_parts = [
        ...
        CO_WEBSITE = company_profile.get("website") or ""
